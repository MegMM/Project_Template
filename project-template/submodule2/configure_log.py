...
class JSONLogger(BaseConfig):
    """ 
        ...
        Called as a) or b): 
            a) logger = JSONLogger(__package__, __name__)
            b) logger = JSONLogger(__package__, __name__, **args)
    ...
    def __init__(self, pkg=None, name=None, **kwargs):
        ...
        #         print(f"{name}: {val!r}")
        setattr(self, 'pkg', pkg if pkg is not None else (WindowsPath(__file__)).anchor)
        setattr(self, 'name', name) if name is not None else ValueError("File name has not been passed to logging")
        setattr(self, 'logger', logging.getLogger(name))
        # print(f"{type(self.LOGCONFIG) = }, {self.LOGCONFIG = }")
        try:
            with open(self.LOGCONFIG, 'r') as f:
                log_config = json.load(f)
        except Exception as e:
            raise Exception(f"Couldn't open log config file: {e}")
        # print(f"{log_config = }")
        log_config['handlers']['file_handler']['filename'] = r"log"
        ...
        try:    
            ...
            raise Exception(f"Couldn't translate JSON config file to dictConfig: {e}")
