...
class Config(UserValidationError):
    def __init__(self):
        super().__init__()
        self.ROOT = WindowsPath(r"C:\MyProjects\Project_Template")
        self.run()
        
    def run(self):
        project_dir = WindowsPath(__file__).parent.name
        if not WindowsPath(__file__).parent/project_dir.is_dir():
            self.name = input(f"Project name: [{project_dir}]: ") or project_dir
